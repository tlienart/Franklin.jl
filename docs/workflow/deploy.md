...
on:
  ...
  pull_request:
    branches:
      - master
```
so build and deploy is also called for PRs.

Then we need to set a local folder to save each website generated by a PR.
...
- name: Fix URLs for PR preview deployment (pull request previews)
  ...
  run: |
    echo "FRANKLIN_WEBSITE_URL=https://{netlify name}.netlify.app/previews/PR${{ github.event.number }}/" >> $GITHUB_ENV
    echo "FRANKLIN_PREPATH=previews/PR${{ github.event.number }}" >> $GITHUB_ENV
```
where `{netlify name}` is a name you will decide at the end of this explanation. 
...
Now for the final step, you will need to visualize the obtained previews. Now since Github only allow you to deploy one branch you will need an alternative like Netlify.
