...
Consider the input directory, consider and process all files (e.g.: convert
all markdown files to html) and write output files at the appropriate place.

* `single_pass` compiles the whole thing once (no dir watching).
* `clear_out_dir` destroys what was previously in the output dir this can be
useful if file names have been changed etc to get rid of stale files.
* `verb` indicates whether to display messages or not.
"""
...
#XXX    set_paths!()
    set_path!()
#XXX    prepare_output_dir(clear_out_dir)
    process_config()
