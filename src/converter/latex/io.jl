...
function lx_input(λ::LxCom)
    ...
    if startswith(qualifier, "plot")
        ...
        s  = split(plot, ":")
        id = length(s) > 1 ? s[2] : ""
        _lx_input_plot(rpath, id)
    end
    # assume it's a language e.g. \input{julia}{ex2}
    return _lx_input_code(rpath; lang=p2, name="input")
end
...
function _lx_input_code(rpath::AS; lang="")::String
    try
        ...
        code = read(fp, String)
        return html_code(code, lang; name="input")
    catch e
        return html_err(e.m)
    end
end
...
function _lx_input_plot(rpath::AS, id::AS="")
    ...
    fname = splitext(splitdir(cp.script_path)[2])[1]
    odir  = cp.out_dir
    pname = fname * id
    ...
    for (root, _, files) in walkdir(odir)
        for (f, e) ∈ splitext.(files)
            ...
            if f == pname && lc_e ∈ (".gif", ".jpg", ".jpeg", ".png", ".svg")
                ...
                ppath  = unixify(joinpath(reldir, plt_name * lc_e))
...
"""
function lx_output(λ::LxCom; reproc::Bool=false, res::Bool=false)
    ...
    reproc || return html_code(output; name="output")
...
"""
lx_textoutput(λ::LxCom) = lx_output(λ; reproc=true)

"""
Same as [`output`](@ref) but adding the result.
"""
lx_show(λ::LxCom) = lx_output(λ; res=true)
...
"""
function lx_textinput(λ::LxCom)
    rpath = strip(content(lxc.braces[1]))
    try
        fp,   = resolve_rpath(rpath)
        input = read(fp, String)
        return reprocess(input)
    catch e
        return html_err(e.m)
    end
end
...
"""
function lx_tableinput(λ::LxCom)
    rpath  = strip(content(lxc.braces[2]))
...
function lx_literate(λ::LxCom)
    ...
    # then reprocess
    return reprocess(read(opath, String))
end
