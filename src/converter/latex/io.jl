...
function lx_input(lxc::LxCom)
    ...
    return _lx_input_code(rpath; lang=qualifier, name="input")
end

"""Helper function to input a script."""
function _lx_input_code(rpath::AS; lang="", name="input")::String
    try
        fp,  = resolve_rpath(rpath, lang)
        code = read(fp, String)
        return html_code(code, lang; name=name)
    catch e
        return html_err(e.m)
    end
end
...
"""
function lx_output(lxc::LxCom; reproc::Bool=false, res::Bool=false)
...
"""
lx_textoutput(lxc::LxCom) = lx_output(lxc; reproc=true)

"""
Same as [`output`](@ref) but adding the result.
"""
lx_show(lxc::LxCom) = lx_output(lxc; res=true)
...
"""
function lx_textinput(lxc::LxCom)
    rpath = strip(content(lxc.braces[1]))
    try
...
"""
function lx_tableinput(lxc::LxCom)
    rpath  = strip(content(lxc.braces[2]))
...
"""
function lx_literate(lxc::LxCom)
    rpath = strip(content(lxc.braces[1]))
