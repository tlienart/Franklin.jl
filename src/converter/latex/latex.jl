...
function resolve_lxcom(lxc::LxCom, lxdefs::Vector{LxDef};
    ...
    name = isnothing(i) ? lxc.ss : subs(lxc.ss, 1, prevind(lxc.ss, first(i)))

    # sort special commands where the input depends on context (see hyperrefs
    # and inputs)
    ...
    # In subsequent case, whatever the command inserts will be re-parsed (in
    # case the insertion contains further commands or markdown); partial
    # corresponds to what the command corresponds to before re-processing.
    partial = ""
    ...
    if name ∈ keys(LXCOM_SIMPLE_REPROCESS)
        ...
        lxdef = getdef(lxc)
        # lxdef = nothing may mean we're inmath & not found, it can likely be
        # a valid math command so  let KaTeX deal with it
...
"""
add_label(λ::LxCom) = "<a id=\"$(refstring(strip(content(λ.braces[1]))))\"></a>"


"""
$(SIGNATURES)

Given a `biblabel` command, update `PAGE_BIBREFS` to keep track of the
...
# $(SIGNATURES)
#
# Internal function to read the content of a script file. See also
...
# $(SIGNATURES)
#
# Internal function to read the content of a script file and highlight it using
# `highlight.js`. See also [`resolve_lx_input`](@ref).
# """
...
Internal function to read the raw output of the execution of a file and display
it in a pre block. See also [`resolve_lx_input`](@ref).
"""
...
Internal function to read a file at a specified path and just plug it in.
(Corresponds to what a simple `\\input` command does in LaTeX).
...
Internal function to read a plot outputted by script `rpath`, possibly named
with `id`. See also [`resolve_lx_input`](@ref). The commands `\\fig` and
`\\figalt` should be preferred.
"""
function resolve_lx_input_plotoutput(rpath::AS, id::AS="")::String
    ...
    isdir(out_path) || throw(ErrorException("I found an input plot but not the output dir."))
    out_file = ""
...
Resolve a command of the form `\\input{code:julia}{path_to_script}` where
`path_to_script` is a valid subpath of `/assets/`. All paths should be
expressed relatively to `/assets/` i.e. `/assets/[subpath]/script.jl`. If
things are not in `assets/`, start the path with `../` to go up one level then
go down for instance `../src/pages/script1.jl`. Forward slashes should also be
used on windows.

Different actions can be taken based on the first bracket:
1. `{julia}` or `{code:julia}` will insert the code in the script with
    appropriate highlighting
2. `{output}` or `{output:plain}` will look for an output file in `/scripts/
    output/path_to_script` and will just print the content in a non-highlighted
    code block
3. `{plot}` or `{plot:id}` will look for a displayable image file (gif, png,
    jp(e)g or svg) in `/assets/[subpath]/script1` and will add an `img` block
    as a result. If an `id` is specified, it will try to find an image with the
    same root name ending with `id.ext` where `id` can help identify a specific
    image if several are generated by the script, typically a number will be
    used.
"""
    ...
    if occursin(":", qualifier)
        ...
        if p1 == "code"
            if p2 ∈ keys(CODE_LANG)
                # these are codes for which we know how they're commented and
                # can use the HIDE trick (see HIGHLIGHT and
                # resolve_lx_input_hlcode)
                return resolve_lx_input_hlcode(rpath, p2)
            else
                # another language descriptor, let the user do that with
                # highlights.js note that the HIDE trick will not work here.
