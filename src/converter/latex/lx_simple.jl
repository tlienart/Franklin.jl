...
Internal function to resolve a `\\output{rpath}` (finds the output of a code
block and shows it). The `reprocess` argument indicates whether the output
should be processed as Franklin-Markdown or inserted as is.
"""
...
Internal function to resolve a `\\show{rpath}` (finds the result of a code
block and shows it). No reprocess for this.
"""
...
Internal function to resolve a `\\textoutput{rpath}` (finds the output of a
code block and shows after treating it as markdown to be parsed).
"""
...
Internal function to resolve a `\\textinput{rpath}` (reads the file and show it
after processing).
"""
...
Internal function to resolve a `\\figalt{alt}{rpath}` (finds a plot and
includes it with alt).
"""
...
Internal function to resolve a `\\tableinput{header}{rpath}` (finds a csv and
includes it with header).
"""
...
single string). If header is empty, the first row of the file will be used for
header.
"""
...
Internal function to resolve a `\\file{name}{rpath}` (finds a file and includes
it with link name). Note that while `\\figalt` is tolerant to extensions not
being specified, this one is not.
"""
...
const LXCOM_SIMPLE_REPROCESS = LittleDict{String, Function}(
    "\\textoutput" => resolve_lx_textoutput, # generated output with reproc
