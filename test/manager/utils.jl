temp_config = joinpath(JuDoc.PATHS[:in], "config.md")
write(temp_config, "@def author = \"Stefan Zweig\"\n")
temp_index = joinpath(JuDoc.PATHS[:in], "index.md")
write(temp_index, "blah blah")
temp_index2 = joinpath(JuDoc.PATHS[:in], "index.html")
write(temp_index2, "blah blih")
temp_blah = joinpath(JuDoc.PATHS[:in_pages], "blah.md")
write(temp_blah, "blah blah")
temp_html = joinpath(JuDoc.PATHS[:in_pages], "temp.html")
write(temp_html, "some html")
temp_rnd = joinpath(JuDoc.PATHS[:in_pages], "temp.rnd")
write(temp_rnd, "some random")
temp_css = joinpath(JuDoc.PATHS[:in_css], "temp.css")
...
@testset "Prep outdir" begin # ✅ aug 15, 2018
    ...
    @test isdir(JuDoc.PATHS[:out])
    @test isdir(JuDoc.PATHS[:out_css])
    temp_out = joinpath(JuDoc.PATHS[:out], "tmp.html")
...
@testset "Scan dir" begin # ✅ aug 16, 2018
    ...
    @test haskey(md_files, JuDoc.PATHS[:in_pages]=>"blah.md")
    @test md_files[JuDoc.PATHS[:in_pages]=>"blah.md"] == mtime(temp_blah) == stat(temp_blah).mtime
    @test html_files[JuDoc.PATHS[:in_pages]=>"temp.html"] == mtime(temp_html)
    @test other_files[JuDoc.PATHS[:in_pages]=>"temp.rnd"] == mtime(temp_rnd)
end
...
@testset "Config+write" begin # ✅ 4 Sept, 2018
    ...
    JuDoc.write_page(JuDoc.PATHS[:in], "index.md", head, pg_foot, foot)
    out_file = joinpath(JuDoc.out_path(JuDoc.PATHS[:f]), "index.html")
...
@testset "Part convert" begin # ✅ 16 aug 2018
    write(joinpath(JuDoc.PATHS[:in_html], "head.html"), raw"""
        <!doctype html>
        <html lang="en-UK">
            <head>
                <meta charset="UTF-8">
        ...
        <body>""")
    write(joinpath(JuDoc.PATHS[:in_html], "page_foot.html"), raw"""
        <div class="page-foot">
                <div class="copyright">
                        &copy; All rights reserved.
                </div>
        </div>""")
    write(joinpath(JuDoc.PATHS[:in_html], "foot.html"), raw"""
            </body>
        </html>""")
    ...
    @test issubset(["css", "libs", "index.html"], readdir(JuDoc.PATHS[:f]))
    @test issubset(["temp.html", "temp.rnd"], readdir(JuDoc.PATHS[:out]))
    @test all(split(read(joinpath(JuDoc.PATHS[:f], "index.html"), String)) .== split("<!doctype html>\n<html lang=\"en-UK\">\n\t<head>\n\t\t<meta charset=\"UTF-8\">\n\t\t<link rel=\"stylesheet\" href=\"/css/main.css\">\n\t</head>\n<body>\n<div class=\"jd-content\">\n<p>blah blah</p>\n\n<div class=\"page-foot\">\n\t\t<div class=\"copyright\">\n\t\t\t\t&copy; All rights reserved.\n\t\t</div>\n</div>\n</div>\n    </body>\n</html>"))
end
...
@testset "Err procfile" begin
    ...
    @test_throws ErrorException JuDoc.process_file_err(:md, JuDoc.PATHS[:in] => "index.md"; clear=false)
end
